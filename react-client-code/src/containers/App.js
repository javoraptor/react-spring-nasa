...
const API_ENDPOINT = process.env.REACT_APP_SECRET_CODE;
...
class App extends Component {
  ...
  onCalendarButtonClick() {
    ...
    fetch(API_ENDPOINT +'/images/date/' + this.state.date + '?cameras=' + list).then((response) => response.json()).then((responseJson) => {
  ...
  onFileButtonClick() {
    ...
    //make rest call here
    fetch(API_ENDPOINT +'9090/images/file' + '?cameras=' + list).then((response) => response.json()).then((responseJson) => {
